...
const displayCard = (displayParameter) => {
    ...
    displayParameter.forEach(items => {
        ...
        if (convertTime !== "") {
            ...
            const hours = Math.floor(parseTime / 3600) %24
            const minutes = Math.floor(parseTime % 3600 / 60)
            const days = Math.floor(Math.floor(parseTime / 3600) /24)
            if(days){
                showTime = days + " days " + hours + " hrs " + minutes + " min ago";
                ...
                <div class="mt-5">
                    <div class="flex gap-3">
                        ...
                        <div>
                            ...
                            <p class="flex items-center gap-2 mt-2 mb-2">${items.authors[0].profile_name}<span> ${items.authors[0].verified ? "<img src='./../images/check.svg' alt=''>" : ""}</span></p>
